...
const ApiContextProvider = props => {
    ...
    useEffect(() => {
        const getMyProfile = async() => {
            try {
                const resMyProf = await axios.get('http://localhost:8000/api/user/myprofile/', {
                    headers: {
                ...
                resMyProf.data[0] && setProfile(resMyProf.data[0]);
                resMyProf.data[0] && setEditedProfile({id:resMyProf.data[0].id, nickName:resMyProf.data[0].nickName});
                resMyProf.data[0] && setAskList(res.data.filter(ask => resMyProf.data[0].userPro === ask.askTo));
    ...
    const newRequestFriend = async(askData) => {
        try {
            const res = await axios.post('http://localhost:8000/api/user/approval/',
                askData,
                {
                    headers: {
                        'Content-Type': 'application/json',
    ...
    const sendDMCont = async(uploadDM) => {
        try {
            await axios.post('http://localhost:8000/api/dm/message/',
                uploadDM,
                {
                    headers: {
                        'Content-Type': 'application/json',
    ...
    const changeApprovalRequest = async(uploadDataAsk, ask) => {
        try {
            ...
            !resp[0] ?
            await axios.post('http://localhost:8000/api/user/approval/',
                newDataAsk,
                {
                    headers: {
                        'Content-Type': 'application/json',
